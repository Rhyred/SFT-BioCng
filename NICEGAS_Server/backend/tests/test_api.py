...
def test_get_devices(client, db_session):
    project_id, _ = seed_test_data(db_session)
    response = client.get(f"/devices?project_id={project_id}")
    assert response.status_code == 200
    data = response.json()
    assert "data" in data
    assert len(data["data"]) == 1

def test_get_device(client, db_session):
...
def test_get_telemetry_missing_filters(client):
    response = client.get("/telemetry")
    assert response.status_code == 422 # FastAPI validation error for missing query params

def test_get_telemetry_invalid_time_range(client, db_session):
    _, device_id = seed_test_data(db_session)
...
def test_get_telemetry_success(client, db_session):
    _, device_id = seed_test_data(db_session)
    now = datetime.now(timezone.utc)
...
def test_get_alerts(client, db_session):
    _, device_id = seed_test_data(db_session)
    response = client.get(f"/alerts?device_id={device_id}")
    assert response.status_code == 200
    data = response.json()
    assert "data" in data
