/**
 * @file node1_biodigester.ino
 * @brief NICEGAS Bio-CNG — NODE 1: BIODIGESTER (ESP32 Standardized Dummy Telemetry Publisher)
 * 
...
const char* DEVICE_ID       = "DIGESTER-01";
const char* DEVICE_NAME     = "ESP32 Biodigester Node";
const char* COMPONENT       = "biodigester";
...
struct BiodigesterMetrics {
    float temperature = 37.50f; // °C   (Range: 36.5 - 38.5)
    float pressure    = 1.30f;  // bar  (Range: 1.20 - 1.40)
    float methane     = 62.0f;  // %    (Range: 60.0 - 64.0)
    float gas_flow    = 25.5f;  // Nm³/h(Range: 23.0 - 28.0)
    float ph          = 7.15f;  // pH   (Range: 6.90 - 7.40)
} metrics;
...
void updateDummyTelemetry() {
    ...
    metrics.temperature = applyBoundedDrift(metrics.temperature, 36.50f, 38.50f, 0.08f);
    metrics.pressure    = applyBoundedDrift(metrics.pressure,    1.20f,  1.40f,  0.01f);
    metrics.methane     = applyBoundedDrift(metrics.methane,     60.00f, 64.00f, 0.15f);
    metrics.gas_flow    = applyBoundedDrift(metrics.gas_flow,    23.00f, 28.00f, 0.20f);
    metrics.ph          = applyBoundedDrift(metrics.ph,          6.90f,  7.40f,  0.02f);
#endif
}
...
void publishTelemetry() {
    ...
    phObj["v"] = round(metrics.ph * 100.0f) / 100.0f;
    phObj["u"] = "pH";
    ...
    if (ok) {
        ...
        Serial.printf("[TELEMETRY] temp=%.2fC pressure=%.2fbar methane=%.1f%% flow=%.1fNm3/h ph=%.2f\n",
                      metrics.temperature, metrics.pressure, metrics.methane, metrics.gas_flow, metrics.ph);
    } else {
...
void setup() {
    ...
    Serial.println("\n==================================================");
    Serial.println("   NICEGAS Bio-CNG — NODE 1: BIODIGESTER        ");
    Serial.println("==================================================");
